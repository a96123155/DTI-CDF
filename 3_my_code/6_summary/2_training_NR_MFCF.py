...
labels = pd.read_csv('./1_original_data/NR/NR_labels.csv')
...
def run_classification_configuration(data,labels,test_idx,trees,c,layer,mode,seed,performance=True,cw=0.001):
    ...
    for fold_data,test_idx_fold in zip(data,test_idx):
        ...
        test_idx_fold = np.array(test_idx_fold['0']) 
        ...
        if performance:
            ...
            AUC_training = roc_auc_score(y_train, y_predprob_train[:,1:2]) 
            AUC_testing = roc_auc_score(y_test, y_predprob_test[:,1:2]) 
            ...
            total_AUC_training += AUC_training 
            total_AUC_testing += AUC_testing  
        else:
            All_scores.append(scores_testing)
    if performance:
        Avg_AUPR_training = 1.0*total_AUPR_training
        Avg_AUPR = 1.0*total_AUPR_testing/len(data)
        Avg_AUC_training = 1.0*total_AUC_training     
        Avg_AUC = 1.0*total_AUC_testing/len(data)       
        
    if performance:
        return [Avg_AUPR_training,Avg_AUPR,folds_AUPR,Avg_AUC_training,Avg_AUC,folds_AUC],test_true_predict_compare   AUC
    else:
...
def run_classification(data,labels,test_idx,learning_rate,max_depth,no_trees,criterion,layer,mode,seed):
    ...
    if layer in ['xgb2','rf1xgb1rf1','xgb3','rf2xgb2rf2','rfxgb2rf2']:
...
def gain_results(seeds,mode,labels,learning_rate,max_depth,no_trees,criterion,layer):
    ...
    for seed in seeds:
        ...
        file_seq_folddata = glob.glob('./NR/NR_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
        file_seq_testidx = glob.glob('./NR/NR_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
...
model_architecture = ['rf1xgb1rf1','rf2xgb2rf2','rfxgb2rf2','rf2','xgb2','xgb3','rf3']
