...
import os
os.chdir('/home/dqw_cyy/11_MFDF_code/1_original_data')
...
labels = pd.read_csv('/home/dqw_cyy/11_MFDF_code/4_v2_generate_dataset/E/D/E_labels.csv')
...
def run_classification_configuration(data,labels,test_idx,trees,c,layer,mode,seed,performance=True,cw=0.001):
    ...
    for fold_data,test_idx_fold in zip(data,test_idx):
        ...
        # 模型写入pickle文件
        with open('/home/dqw_cyy/11_MFDF_code/10_model/E/gc_model_MFCF/rf2xgb2rf2/model_E_MFCF_' + layer + '_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.pkl', "wb") as f1:
            pickle.dump(gc, f1, pickle.HIGHEST_PROTOCOL)
        with open('/home/dqw_cyy/11_MFDF_code/10_model/E/MaxAbsScaler_MFCF/rf2xgb2rf2/MaxAbsScaler_E_MFCF_' + layer + '_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.pkl'.format(i), "wb") as f2:
...
def run_classification(data,labels,test_idx,learning_rate,max_depth,no_trees,criterion,layer,mode,seed):
    ...
    if layer in ['xgb2','rf1xgb1rf1','xgb3','rfxgb2rf']:#'rf2xgb2rf2',
...
def gain_results(seeds,mode,labels,learning_rate,max_depth,no_trees,criterion,layer):
    ...
    for seed in seeds:
        ...
        file_seq_folddata = glob.glob('/home/dqw_cyy/11_MFDF_code/4_v2_generate_dataset/E/D/E_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
        file_seq_testidx = glob.glob('/home/dqw_cyy/11_MFDF_code/4_v2_generate_dataset/E/D/E_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
...
with open('/home/dqw_cyy/11_MFDF_code/9_y_pred/E/0_summary__E_MFCF_rf2xgb2rf2_SD-err_auc.csv', 'wb') as f:
