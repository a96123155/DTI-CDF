...
import sys
sys.path.append('/data/dqw_cjy/11_MFDF_code/3_my_code')
import os
os.chdir('/data/dqw_cjy/11_MFDF_code/1_original_data')
...
labels = pd.read_csv('/data/dqw_cjy/11_MFDF_code/4_generate_dataset/E/E_labels.csv')
...
def run_classification_configuration(data,labels,test_idx,trees,c,layer,mode,seed,performance=True,cw=0.001):
    i = 0
    All_scores = []
    ...
    for fold_data,test_idx_fold in zip(data,test_idx):
        ...
        # 模型写入pickle文件
        with open('/data/dqw_cjy/11_MFDF_code/10_model/E/gc_model_MFCF/rf2xgb2rf2/model_E_MFCF_' + layer + '_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.pkl', "wb") as f1:
            pickle.dump(gc, f1, pickle.HIGHEST_PROTOCOL)
        with open('/data/dqw_cjy/11_MFDF_code/10_model/E/MaxAbsScaler_MFCF/rf2xgb2rf2/MaxAbsScaler_E_MFCF_' + layer + '_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.pkl'.format(i), "wb") as f2:
            pickle.dump(max_abs_scaler, f2, pickle.HIGHEST_PROTOCOL)
        i += 1
...
def run_classification(data,labels,test_idx,learning_rate,max_depth,no_trees,criterion,layer,mode,seed):
    ...
    if layer in ['xgb2','rf1xgb1rf1','xgb3','rfxgb2rf']:
        for cw in learning_rate:
            for t in no_trees:
                parameter_result,test_true_predict_compare = run_classification_configuration(data,labels,test_idx,t,c,layer,mode,seed,cw)
        ...
        for t in no_trees:
            parameter_result,test_true_predict_compare = run_classification_configuration(data,labels,test_idx,t,c,layer,mode,seed)
            result.append(parameter_result)
            test_true_predict_compare_10cv.append(test_true_predict_compare)
...
def gain_results(seeds,mode,labels,learning_rate,max_depth,no_trees,criterion,layer):
    ...
    for seed in seeds:
        ...
        file_seq_folddata = glob.glob('/data/dqw_cjy/11_MFDF_code/4_generate_dataset/E/E_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
        file_seq_testidx = glob.glob('/data/dqw_cjy/11_MFDF_code/4_generate_dataset/E/E_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
        ...
        results,parameter_result_df,test_true_predict_compare_10cv = run_classification(data,labels,test_idx,learning_rate,max_depth,no_trees,criterion,layer,mode,seed)
...
with open('/data/dqw_cjy/11_MFDF_code/9_y_pred/E/025_summary__E_MFCF_rf2xgb2rf2_ST-err_auc.csv', 'wb') as f:
