...
import sys
sys.path.append('/opt/yanyichu/MFDF/0_my_code')
import os
os.chdir('/opt/yanyichu/MFDF/1_original_data')
...
labels = pd.read_csv('/opt/yanyichu/MFDF/4_generate_dataset_MFCF/E/E_labels.csv')
...
def run_classification_configuration(data,labels,test_idx,trees,c,layer,mode,seed,performance=True,cw=0.001):
    ...
    for fold_data,test_idx_fold in zip(data,test_idx):
        ...
        # 模型写入pickle文件
        with open('/opt/yanyichu/MFDF/10_model/E/gc_model_MFCF/rf2xgb2rf2/model_E_MFCF_' + layer + '_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.pkl', "wb") as f1:
            pickle.dump(gc, f1, pickle.HIGHEST_PROTOCOL)
        with open('/opt/yanyichu/MFDF/10_model/E/MaxAbsScaler_MFCF/rf2xgb2rf2/MaxAbsScaler_E_MFCF_' + layer + '_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.pkl'.format(i), "wb") as f2:
...
def gain_results(seeds,mode,labels,learning_rate,max_depth,no_trees,criterion,layer):
    ...
    for seed in seeds:
        ...
        file_seq_folddata = glob.glob('/opt/yanyichu/MFDF/4_generate_dataset_MFCF/E/E_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
        file_seq_testidx = glob.glob('/opt/yanyichu/MFDF/4_generate_dataset_MFCF/E/E_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
...
with open('/opt/yanyichu/MFDF/9_y_pred/E/0_summary__E_MFCF_rf2xgb2rf2_ST-err_auc.csv', 'wb') as f:
