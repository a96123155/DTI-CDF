...
def run_classification_configuration(data,labels,test_idx,trees,c,layer,mode,seed,performance=True,cw=0.001):
    ...
    for fold_data,test_idx_fold in zip(data,test_idx):
        ...
        # 模型写入pickle文件
        with open('/home/dqw_cyy/11_MFDF_code/10_model/E/gc_model_MFCF/rf3/model_E_MFCF_' + layer + '_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.pkl', "wb") as f1:
            pickle.dump(gc, f1, pickle.HIGHEST_PROTOCOL)
        with open('/home/dqw_cyy/11_MFDF_code/10_model/E/MaxAbsScaler_MFCF/rf3/MaxAbsScaler_E_MFCF_' + layer + '_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.pkl'.format(i), "wb") as f2:
...
with open('/data/dqw_cjy/11_MFDF_code/9_y_pred/0_summary__E_MFCF_rf3_ST-err_auc.csv', 'wb') as f:
