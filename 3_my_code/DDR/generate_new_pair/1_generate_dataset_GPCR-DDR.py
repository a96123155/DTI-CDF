...
import os
os.chdir('/home/yanyichu/11_MFDF_code/1_original_data/GPCR')
...
R_all_train_test = "GPCR_combine_origin_new_interactions.txt"
...
labels = mat2vec(DT)
pd.DataFrame(labels).to_csv('/home/yanyichu/11_MFDF_code/4_generate_dataset_DDR/GPCR/GPCR_DDR_labels.csv', index = False)
...
for mode in mode_list:
    ...
    for seed in seeds:
        ...
        for fold_data,test_idx_fold in zip(folds_features,test_idx):
            pd.DataFrame(fold_data).to_csv('/home/yanyichu/11_MFDF_code/4_generate_dataset_DDR/GPCR/GPCR_DDR_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.csv',index = False)
            pd.DataFrame(test_idx_fold).to_csv('/home/yanyichu/11_MFDF_code/4_generate_dataset_DDR/GPCR/GPCR_DDR_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.csv', index = False)
            i += 1
