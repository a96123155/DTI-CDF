...
labels = pd.read_csv(r'E:\11_MFDF_code\4_generate_dataset_DDR\NR\NR_DDR_labels.csv')
...
def gain_results(seeds,mode,labels,learning_rate,max_depth,no_trees,criterion,layer):
    ...
    for seed in seeds:
        ...
        file_seq_folddata = glob.glob(r'E:\11_MFDF_code\4_generate_dataset_DDR\NR\NR_DDR_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
        file_seq_testidx = glob.glob(r'E:\11_MFDF_code\4_generate_dataset_DDR\NR\NR_DDR_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
...
with open(r'E:\11_MFDF_code\9_y_pred\6.1_training_NR_DDR_rf1xgb2rf1.csv', 'wb') as f:
...
with open(r'E:\11_MFDF_code\9_y_pred\6.1_training_NR_DDR_rf1xgb1rf1.csv', 'wb') as f:
    wr = csv.writer(f, quoting=csv.QUOTE_ALL)
...
# ('model_architecture:', 'rf2')
# Mode: p
# Average AUPR: 0.96436659598308
# Average AUC: [0.979995094279292]
# seed_results: 
# ('Avg_AUPR_training:', 9.983366295657824)
# ('Avg_AUPR:', 0.9643665959830798)
# ('folds_AUPR:', [0.9723214285714286, 0.9962086397058824, 1.0, 1.0, 1.0, 1.0, 0.9132936507936508, 0.9650651927437641, 0.9089478918347965, 0.8878291561812782])
# ('Avg_AUC_training:', 9.992597546068732)
# ('Avg_AUC:', 0.979995094279292)
# ('folds_AUC:', [0.96875, 0.9994959677419355, 1.0, 1.0, 1.0, 1.0, 0.9373861566484518, 0.9876302083333333, 0.973922902494331, 0.9327657075748679])
...
for mode in mode_list:
    ...
    for seed in range(len(test_true_predict_compare_10cv_seeds_modes[0])):
        for fold in range(len(test_true_predict_compare_10cv_seeds_modes[0][0])):
            y_compare_df = pd.DataFrame(test_true_predict_compare_10cv_seeds_modes[0][seed][fold]).T
    ...
    plt.plot(mean_recall,mean_precision, color='r',label=r'Mean P-R (AUPR = %0.2f $\pm$ %0.2f)' % (mean_auc, std_auc),lw=2.6, alpha=.8)
    ...
    plt.savefig(r'E:\11_MFDF_code\8_figure\v1_NR_%s_DDR_AUPR' % mode)
    plt.show()
...
for mode in mode_list:
    ...
    for seed in range(len(test_true_predict_compare_10cv_seeds_modes[0])):
        for fold in range(len(test_true_predict_compare_10cv_seeds_modes[0][0])):
            ...
            roc_auc = auc(fpr, tpr)
            aucs.append(roc_auc)
            #plt.plot(fpr, tpr, lw=1, alpha=0.9,label='ROC fold %d (AUC = %0.2f)' % (i, roc_auc))
    ...
    std_auc = np.std(aucs)
    plt.plot(mean_fpr, mean_tpr, color='r',label=r'Mean ROC (AUC = %0.2f $\pm$ %0.2f)' % (mean_auc, std_auc),lw=2.6, alpha=.8)
    ...
    plt.savefig(r'E:\11_MFDF_code\8_figure\v1_NR_%s_AUC_1' % mode)
    plt.show()
