...
protein_psepssm_file = pd.read_csv(r'E:\11_MFDF_code\2_feature\NR\nr_psepssm.csv')
molecule_fp2_file = pd.read_csv(r'E:\11_MFDF_code\2_feature\NR\NR_fp2.csv')
...
labels = mat2vec(DT)
pd.DataFrame(labels).to_csv(r'E:\11_MFDF_code\4_generate_dataset\NR\NR_labels.csv', index = False)
...
for mode in mode_list:
    ...
    for seed in seeds:
        ...
        for fold_data,test_idx_fold in zip(folds_features,test_idx):
            pd.DataFrame(fold_data).to_csv(r'E:\11_MFDF_code\4_generate_dataset\NR\NR_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.csv',index = False)
            pd.DataFrame(test_idx_fold).to_csv(r'E:\11_MFDF_code\4_generate_dataset\NR\NR_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.csv', index = False)
            i += 1
...
len(folds_features) #10 == 10-fold cross-validation)
len(folds_features[0]) # 1404 == 1314+90 (样本数)
len(folds_features[0][0]) #488 == 12+256+220 (三种特征组合数numpy.float64) 
#len(folds_features[0][0][0]) #TypeError: object of type 'numpy.float64' has no len()
folds_features[0][0]


# In[35]:

pd.DataFrame(folds_features[0])


# In[38]:

len(labels) #1404 (numpy.float64)
pd.DataFrame(labels)
