...
labels = mat2vec(DT)
pd.DataFrame(labels).to_csv('/lustre/home/acct-clsdqw/clsdqw-tahir/dqw_cyy/11_MFDF_code/4_generate_dataset_DDR/IC/IC_DDR_labels.csv', index = False)
...
for mode in mode_list:
    ...
    for seed in seeds:
        ...
        for fold_data,test_idx_fold in zip(folds_features,test_idx):
            pd.DataFrame(fold_data).to_csv('/lustre/home/acct-clsdqw/clsdqw-tahir/dqw_cyy/11_MFDF_code/4_generate_dataset_DDR/IC/IC_DDR_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.csv',index = False)
            pd.DataFrame(test_idx_fold).to_csv('/lustre/home/acct-clsdqw/clsdqw-tahir/dqw_cyy/11_MFDF_code/4_generate_dataset_DDR/IC/IC_DDR_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.csv', index = False)
            i += 1
