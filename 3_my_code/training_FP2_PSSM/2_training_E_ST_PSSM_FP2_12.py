...
for i in range(1000):
    print 'couzicocuziCOUZEOUZI',
print()

# In[8]:

labels = pd.read_csv('/lustre/home/acct-clsdqw/clsdqw-tahir/dqw_cyy/11_MFDF_code/4_generate_dataset/E/E_labels.csv')
...
def gain_results(seeds,mode,labels,learning_rate,max_depth,no_trees,criterion,layer):
    ...
    for seed in seeds:
        ...
        file_seq_folddata = glob.glob('/lustre/home/acct-clsdqw/clsdqw-tahir/dqw_cyy/11_MFDF_code/4_generate_dataset/E/E_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
        file_seq_testidx = glob.glob('/lustre/home/acct-clsdqw/clsdqw-tahir/dqw_cyy/11_MFDF_code/4_generate_dataset/E/E_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold*' + '.csv')
