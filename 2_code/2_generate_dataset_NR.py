...
protein_psepssm_file = pd.read_csv('1_original_data/NR/nr_psepssm.csv')
molecule_fp2_file = pd.read_csv('1_original_data/NR/NR_fp2.csv')
...
labels = mat2vec(DT)
pd.DataFrame(labels).to_csv('NR_labels.csv', index = False)
...
for mode in mode_list:
    ...
    for seed in seeds:
        ...
        for fold_data,test_idx_fold in zip(folds_features,test_idx):
            pd.DataFrame(fold_data).to_csv('./NR/NR_folddata_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.csv',index = False)
            pd.DataFrame(test_idx_fold).to_csv('./NR/NR_testidx_S' + str(mode) + '_seed' + str(seed) + '_fold' + str(i) + '.csv', index = False)
            i += 1